...
app.get("/mascotas/:nombre", (req, res) => {
    try {
        ...
        let filterMascotas = data.mascotas.filter(mascota => mascota.nombre.toUpperCase() == nombre.toUpperCase())
...
app.post("/mascotas", (req, res) => {
    try {
        ...
        if(!mascota || !run || !propietario){
           return res.status(400).json({code: 400, message:"Debe enviar todos los datos requeridos [nombre mascota, run propietario, nombre propietario]"})
        }
        let nuevaMascota = {
        id: uuid().slice(0,6),
        nombre: mascota,
...
app.delete("/mascotas/:nombre", (req, res) => {
    ...
    try{
        ...
        if(eliminarMascota(nombre)){
           res.json({code: 200, message:`Mascota ${nombre} eliminada correctamete del registro`}) 
        }else{
...
app.delete("/mascotas/propietario/:run", (req, res) => {
    ...
    try{
        ...
        if(eliminarMascotaPorRun(run)){
           res.json({code: 200, message:`Mascota/s del propietario con RUN ${run} eliminadas correctamete del registro`}) 
        }else{
